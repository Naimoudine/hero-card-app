...
export default function AddCollectionModal({ modal, setCollection }) {
  ...
  useEffect(() => { inputRef.current.select() }, [modal])

  return (
    <div className="absolute top-0 left-0 flex items-center justify-center w-screen h-screen bg-black/80" id="backdrop" onClick={e => onCloseModal(e)}>
      <div className="p-10 text-white rounded-lg bg-gray-950/90 backdrop-blur-sm">
        <input type="text" name="title" className="bg-transparent border-b border-gray-500 outline-none" placeholder="Title" onChange={e => setTitle(e.target.value)} ref={inputRef} />
        <div className="relative flex items-center mt-8 gap-x-6">
          ...
          <input className="bg-transparent border-b border-gray-500 w-[80px] outline-none" type="text" name="" id="" placeholder="hex code" onChange={e => setColor(e.target.value)} />
        </div>
