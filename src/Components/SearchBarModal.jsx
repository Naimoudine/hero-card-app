  ...
  return (
    <div className={modal ? `absolute z-50 w-screen h-screen flex justify-center pt-48 bg-black/60 backdrop-blur-[2px]` : `hidden`} onClick={e => closeModal(e)} id="backdrop">
      <div className="w-[50%] h-fit">
        <div className={heroList ? `w-full h-fit bg-white flex items-center justify-end py-4 px-4 rounded-t-lg` : `w-full h-fit bg-white flex items-center justify-end py-4 px-4 rounded-lg`}>
          <div className="flex items-center w-full">
            ...
            <input type="text" className=" w-[90%] outline-none" value={searchValue} onChange={e => setSearchValue(e.target.value)} autoselect={true} ref={inputRef} />
          </div>
          <span>Enter</span>
        </div>
        <div className={heroList ? `bg-white max-h-[350px] w-full overflow-scroll rounded-b-lg` : `hidden`}>
          {heroList?.map(hero => <SearchItem key={hero?.id} data={hero} setSelectedHeroData={setSelectedHeroData} setModal={setModal} setSearchValue={setSearchValue} />)}
        </div>
      </div>
    </div>
  )
